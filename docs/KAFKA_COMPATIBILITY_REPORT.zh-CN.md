...
- 本项目基准：`master`，提交 `e7809d0e28a35b66de80bcd34d5681c453dadffd`。
...
| 普通自动化测试 | 88 个通过 | 81 个 library unit tests + 7 个 CLI tests；两个真实 Kafka 测试默认 ignored，由 CI 运行 |
...
缺少：Java 自定义 line reader class。其他底层 producer 配置仍可通过 `--command-property key=value` 传入。
...
- Rust 单元测试与普通 CLI 测试：88 个通过（81 个 library unit tests + 7 个 CLI tests）。
...
当前实现基准 `e7809d0` 已由 GitHub Actions 运行
[`30833554681`](https://github.com/lihongjie0209/kafka-cli/actions/runs/30833554681) 完整验证通过：fmt/Clippy/88 个普通测试、bundled glibc、Kafka 3.6.2、包含 console reader/formatter properties 文件、规范 property 名称、gzip 缺省、batch 覆盖和 max-block 参数的实际生产消费，以及 broker default、quota、broker-logger、client-metrics、fenced endpoint、原版 log-dirs describe 语法与 reassignment 闭环的 Kafka 4.3.1、x86_64 musl 和 aarch64 musl。普通 CLI 测试会通过真实兼容符号链接启动二进制，并验证 configs `--alter` 会进入执行路径而非输出 PREVIEW；单元测试逐项覆盖 producer 配置映射、配置文件合并、废弃配置别名、groups/configs/ACL/reassignment，以及 delete-records、leader-election、cluster unregister 的 mutation rewrite。配置写入后的集成断言采用最多 5 秒的有界重试处理 Kafka 配置传播，超时仍会保留最后一次实际输出并使测试失败。
