...
- 本项目基准：`master`，提交 `d79cf543b956e15ebebb9af329af5c94c519614c`。
...
| 普通自动化测试 | 132 个通过 | 125 个 library unit tests + 7 个 CLI tests；两个真实 Kafka 测试默认 ignored，由 CI 运行 |
...
- Rust 单元测试与普通 CLI 测试：132 个通过（125 个 library unit tests + 7 个 CLI tests）。
...
当前实现基准 `d79cf54` 已由 GitHub Actions 运行
[`30841700654`](https://github.com/lihongjie0209/kafka-cli/actions/runs/30841700654) 完整验证通过：fmt/Clippy/128 个普通测试、bundled glibc、Kafka 3.6.2、Kafka 4.3.1、x86_64 musl 和 aarch64 musl。GetOffsetShell 和 console consumer 的边界语义持续回归；ConfigCommand 新增单次逗号列表删除两个配置的真实闭环。两种 musl artifact 均执行 `--version`/`--help`，ARM64 通过 QEMU user-mode emulator 启动。CI Actions 使用 Node.js 24 主版本；Zig 0.15.2 从官方 release index 获取并校验 SHA-256。该运行的六个 job 全部成功。配置写入后的集成断言采用最多 5 秒的有界重试处理 Kafka 配置传播，超时仍会保留最后一次实际输出并使测试失败。
