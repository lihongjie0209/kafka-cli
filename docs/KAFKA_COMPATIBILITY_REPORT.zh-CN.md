...
| 2026-08-04 | ConsumerGroup Kafka 4 状态边界 | `Assigning`/`Reconciling` 从笼统 unknown 改为请求前明确报告 librdkafka 2.12 能力限制；确认 `NotReady` 与 Share/Streams 不属于原版 consumer-groups 的合法过滤集合 | 149 个单元测试、7 个 CLI 测试；与 Kafka 4.4 `groupStatesFromString`/`consumerGroupTypesFromString` 源码差分核对 |
