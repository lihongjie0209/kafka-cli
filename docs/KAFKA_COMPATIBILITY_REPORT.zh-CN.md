...
| 普通自动化测试 | 226 个通过 | 211 个 library unit tests + 15 个 CLI tests；两个真实 Kafka 测试默认 ignored，由 CI 运行 |
...
- Rust 单元测试与普通 CLI 测试：226 个通过（211 个 library unit tests + 15 个 CLI tests）。
