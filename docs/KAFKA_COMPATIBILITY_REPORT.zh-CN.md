...
- 本项目审计实现基准：`master`，提交 `8cba92f80f698840a68d96fb771f0e5b723082e8`。
...
当前审计实现基准 `8cba92f` 已由 GitHub Actions 运行
[`30843159263`](https://github.com/lihongjie0209/kafka-cli/actions/runs/30843159263) 完整验证通过：fmt/Clippy/134 个普通测试、bundled glibc、Kafka 3.6.2、Kafka 4.3.1、x86_64 musl 和 aarch64 musl。Kafka 4.3.1 覆盖 ACL Match 查询、双 host 精确删除，以及三资源批量 add/list/remove 闭环。两种 musl artifact 均执行 `--version`/`--help`，ARM64 通过 QEMU user-mode emulator 启动。CI Actions 使用 Node.js 24 主版本；Zig 0.15.2 从官方 release index 获取并校验 SHA-256。该运行的六个 job 全部成功。配置写入后的集成断言采用最多 5 秒的有界重试处理 Kafka 配置传播，超时仍会保留最后一次实际输出并使测试失败。
