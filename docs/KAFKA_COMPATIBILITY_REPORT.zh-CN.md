...
- 本项目审计实现基准：`master`，提交 `cf77ba80a861435575b9d7522dbaee5712aff1be`。
...
当前审计实现基准 `cf77ba8` 已由 GitHub Actions 运行
[`30848663059`](https://github.com/lihongjie0209/kafka-cli/actions/runs/30848663059) 完整验证通过：fmt/Clippy/158 个普通测试、bundled glibc、Kafka 3.6.2、Kafka 4.3.1、x86_64 musl 和 aarch64 musl。Kafka 4.3.1 覆盖 ACL、Group、Config、offset 等既有闭环，并真实执行 Kafka 4 `groups list --state Assigning` 及 verbose state/member epoch 协议 fallback；Kafka 3.6.2 回归同时通过。两种 musl artifact 均执行 `--version`/`--help`，ARM64 通过 QEMU user-mode emulator 启动。CI Actions 使用 Node.js 24 主版本；Zig 0.15.2 从官方 release index 获取并校验 SHA-256。该运行的六个 job 全部成功。配置写入后的集成断言采用最多 5 秒的有界重试处理 Kafka 配置传播，超时仍会保留最后一次实际输出并使测试失败。
